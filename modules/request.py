...
class Requests:
    ...
    def download_image(self, title, image_url, download_directory, is_poster=True, filename=None):
        response = self.get(image_url, header=True)
    ...
    def file_yaml(self, path_to_file, check_empty=False, create=False):
        return YAML(path=path_to_file, check_empty=check_empty, create=create)
